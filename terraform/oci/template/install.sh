...
read -rp "Enter your OCI Compartment ID: " compartment_id

# Validate input (ensure it's not empty)
if [[ -z "$compartment_id" ]]; then
    echo "Error: Compartment ID cannot be empty."
    exit 1
fi

# Prompt for S3 API Compatibility Access Key ID
read -rp "Enter your S3 API Compatibility Access Key ID: " aws_access_key_id
...
# Prompt for S3 API Compatibility Access Secret ID
read -rp "Enter your S3 API Compatibility Access Secret ID): " aws_secret_access_key

# Validate input (ensure it's not empty)
if [[ -z "$aws_secret_access_key" ]]; then
    echo "Error: Access Key Secret ID cannot be empty."
    exit 1
fi

function create_tf_backend() {
    echo -e "Creating terraform state backend"
    bash create_tf_backend.sh "$environment" "$compartment_id" "$aws_access_key_id" "$aws_secret_access_key"
}
